...
func New(addr string, opt ...Option) *Client {
	...
	fmt.Println("新连接", addr, opts.UID, opts.Token, opts.IsDebug)
...
func (c *Client) Connect() error {
	...
	fmt.Println("is Debug", c.isDebug)
	if c.isDebug {
...
func (c *Client) processConnectInit() error {
	if c.isDebug {
		fmt.Println("client processConnectInit")
	}
...
func (c *Client) kickFlusher() {
	if c.isDebug {
		fmt.Println("client kickFlusher")
	}
	if c.writer != nil {
		select {
...
func (c *Client) processPingTimer() {
	if c.isDebug {
		fmt.Println("client processPingTimer", c.status)
	}
	c.mu.Lock()
	...
	c.pingOut++
	if c.isDebug {
		fmt.Println("client processPingTimer", c.pingOut, c.opts.MaxPingCount)
	}
...
func (c *Client) doReconnect() {
	if c.isDebug {
		fmt.Println("client doReconnect")
	}
...
func (c *Client) FlushTimeout(timeout time.Duration) (err error) {
	if c.isDebug {
		fmt.Println("client FlushTimeout", timeout)
	}
	if timeout <= 0 {
...
func (c *Client) removeFlushEntry(ch chan struct{}) bool {
	if c.isDebug {
		fmt.Println("client removeFlushEntry", ch)
	}
	c.mu.Lock()
	defer c.mu.Unlock()
	if c.pongs == nil {
...
func (c *Client) waitForExits() {
	if c.isDebug {
		fmt.Println("client waitForExits")
	}
...
func (c *Client) processOpErr(err error) {
	if c.isDebug {
		fmt.Println("client processOpErr")
	}
	c.mu.Lock()
	if c.isConnecting() || c.isReconnecting() { //|| c.isClosed()
		c.mu.Unlock()
		return
	}
...
func (c *Client) stopPingTimer() {
	if c.isDebug {
		fmt.Println("client stopPingTimer")
	}
	if c.pingIntervalTimer != nil {
...
func (c *Client) handlePacketDatas(packetData []byte) error {
	if c.isDebug {
		fmt.Println("client handlePacketDatas", packetData)
	}
...
func (c *Client) handlePacket(frame wkproto.Frame) error {
	if c.isDebug {
		fmt.Println("client handlePacket", frame)
	}
...
func (c *Client) handleRecvPacket(packet *wkproto.RecvPacket) {
	if c.isDebug {
		fmt.Println("client handleRecvPacket", packet)
	}
	var err error
	var payload []byte
	if c.onRecv != nil {
...
func (c *Client) SendMessage(channel *Channel, payload []byte, opt ...SendOption) error {
	if c.isDebug {
		fmt.Println("send message", channel)
	}
	opts := NewSendOptions()
	if len(opt) > 0 {
		for _, op := range opt {
	...
	if !opts.NoEncrypt {
		...
		if err != nil {
			if c.isDebug {
				fmt.Println("加密消息payload失败！", zap.Error(err))
			}
	...
	packet.RedDot = true
	fmt.Println("packet pre data", packet)
	// 加密消息通道
	if !opts.NoEncrypt {
		...
		if err != nil {
			if c.isDebug {
				fmt.Println("加密数据失败！！", zap.Error(err))
			}
...
func (c *Client) sendConnect() error {
	...
	c.status = CONNECTED //没改变状态 不能触发心跳
	c.salt = connack.Salt
...
func (c *Client) sendPacket(packet wkproto.Frame) error {
	if c.isDebug {
		fmt.Println("client send sendPacket")
	}
...
func (c *Client) appendPacket(packet wkproto.Frame) error {
	if c.isDebug {
		fmt.Println("client appendPacket")
	}
	data, err := c.proto.EncodeFrame(packet, c.opts.ProtoVersion)
	if err != nil {
		return err
...
func NewChannel(channelID string, channelType uint8) *Channel {
	...
	fmt.Println("new Channel", channelID, channelType)
	return &Channel{
		ChannelID:   channelID,
